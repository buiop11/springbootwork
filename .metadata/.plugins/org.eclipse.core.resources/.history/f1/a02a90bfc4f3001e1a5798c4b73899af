...
//		console.log(error);
		if(error.data == null){
			alert(error.responseJSON.message);  // message만 있는 경우 
		}else{
			alert(JSON.stringify(error.responseJSON.data));  // errorMap이 있으면 그대로 
		}
		
//		alert(error.responseJSON.data.name); // 1. 글자만 그대로   - errorMap 
